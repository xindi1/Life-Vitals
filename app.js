...
function bindDateFormatting() {
  ["filterDate", "sleepDate", "exerciseDate", "nutritionDate", "sexDate"].forEach((id) => {
    ...
    el.addEventListener("blur", () => {
      ...
      if (id === "filterDate" && el.value && !isValidDate(el.value)) {
        el.value = "";
      }
    });
  });
...
function historyCard(module, entry) {
  ...
  const body = {
    sleep: [entry.notes, entry.dreams].filter(Boolean).join("\n"),
    exercise: entry.notes || "",
    nutrition: [entry.notes, entry.hydration, entry.cravings].filter(Boolean).join("\n"),
    sex: [entry.notes, entry.tone].filter(Boolean).join("\n")
  }[module];
