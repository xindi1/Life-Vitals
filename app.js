...
function seedDefaultDates() {
  ...
  ["sleepDate", "exerciseDate", "nutritionDate", "sexDate"].forEach((id) => {
  ...
  if (filterDate && !filterDate.value) filterDate.value = "";
}

function bindDateFormatting() {
  ["filterDate", "sleepDate", "exerciseDate", "nutritionDate", "sexDate"].forEach((id) => {
    const el = document.getElementById(id);
    if (!el) return;
...
function handleSleepSubmit(event) {
  ...
  if (!isValidDate(values.date) || !isValidTime(values.bedtime) || !isValidTime(values.wakeTime) || !values.quality) return;
...
function handleExerciseSubmit(event) {
  ...
  if (!isValidDate(values.date) || !isValidTime(values.time) || !values.type || !values.duration || !values.intensity || !values.regulation) return;
...
function handleNutritionSubmit(event) {
  ...
  if (!isValidDate(values.date) || !isValidTime(values.time) || !values.title || !values.mealType) return;
...
function handleSexSubmit(event) {
  ...
  if (!isValidDate(values.date) || !isValidTime(values.time) || !values.libido || !values.activityType || !values.tone || !values.satisfaction) return;
...
function renderSummary() {
  ...
  const nutritionCount = todayNutrition.length;
  const libidoAvg = todaySex.length
    ? (todaySex.reduce((sum, item) => sum + Number(item.libido || 0), 0) / todaySex.length).toFixed(1)
    : "—";
  const totalEntries = todaySleep.length + todayExercise.length + todayNutrition.length + todaySex.length;
  ...
  summaryCards.innerHTML = `
    ...
    <div class="summary-item"><span class="label">Nutrition entries</span><strong>${nutritionCount}</strong></div>
    <div class="summary-item"><span class="label">Sex entries</span><strong>${todaySex.length}</strong></div>
  `;
  ...
  document.getElementById("todayTotalCount").textContent = String(totalEntries);
  ...
  state.data.sleep.forEach((entry) => {
    if (entry.date === today && entry.bedtime) {
      items.push({
        ...
        time: entry.bedtime,
        title: "Bedtime",
        subtitle: entry.totalHours ? `Estimated sleep: ${entry.totalHours} hrs` : (entry.quality ? `Quality: ${entry.quality}/5` : ""),
        body: entry.notes || ""
      });
    }
    if (entry.date === today && entry.wakeTime) {
      items.push({
        ...
        title: "Wake time",
        subtitle: entry.totalHours ? `Estimated sleep: ${entry.totalHours} hrs` : (entry.quality ? `Quality: ${entry.quality}/5` : ""),
        body: entry.dreams ? `Dreams: ${entry.dreams}${entry.notes ? `\n${entry.notes}` : ""}` : (entry.notes || "")
      });
    }
  });
  ...
  state.data.exercise.forEach((entry) => {
    if (entry.date === today) {
      items.push({
        ...
        subtitle: `${entry.duration || "—"} min${entry.intensity ? ` • Intensity ${entry.intensity}/5` : ""}`,
        body: entry.notes || ""
      });
    }
  });
  ...
  state.data.nutrition.forEach((entry) => {
    if (entry.date === today) {
      items.push({
        module: "nutrition",
        time: entry.time || "00:00",
        title: entry.title || "Nutrition",
        subtitle: entry.mealType || "",
        body: [entry.notes, entry.hydration ? `Hydration: ${entry.hydration}` : "", entry.cravings ? `Cravings: ${entry.cravings}` : ""].filter(Boolean).join("\n")
      });
    }
  });
  ...
  state.data.sex.forEach((entry) => {
    if (entry.date === today) {
      items.push({
        module: "sex",
        time: entry.time || "00:00",
        title: entry.activityType || "Sex",
        subtitle: [entry.libido ? `Libido ${entry.libido}/5` : "", entry.tone || "", entry.satisfaction ? `Satisfaction ${entry.satisfaction}/5` : ""].filter(Boolean).join(" • "),
        body: entry.notes || ""
      });
    }
  });
...
function exportData() {
  const payload = { app: "Life Vitals", exportedAt: new Date().toISOString(), data: state.data };
  const blob = new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" });
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = url;
  link.download = `life-vitals-export-${todayString()}.json`;
  link.click();
...
function importData(event) {
  ...
  reader.onload = () => {
    try {
      ...
      alert("Import failed. Please use a valid Life Vitals JSON export.");
    } finally {
      event.target.value = "";
    }
  };
...
function jumpToTodayView() {
  state.filters = { module: "all", date: todayString(), text: "" };
  document.getElementById("filterModule").value = "all";
